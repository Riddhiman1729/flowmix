...
##'   admm iterations.
##' @param censor_indicator_left list of lower censoring indicators per dimension at each time point tt.
##' @param censor_indicator_right list of upper censoring indicators per dimension at each time point tt.
...
##' LA (locally adaptive) ADMM wrapper to \code{admm_oneclust_flowcut()}.
##'
...
la_admm_oneclust_flowcut <- function(K,
  ...
  for(kk in 1:K){
    ...
    call <- as.call(c(list(as.name("admm_oneclust_flowcut")), argn))
    res = eval(call, args)
    #debug(admm_oneclust_flowcut)
    #res <- do.call(admm_oneclust_flowcut, args)
