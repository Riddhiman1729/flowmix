...
##' @param countslist Counts or biomass.
##' @param X Covariates.
##' @param cens_ind_left the left censoring indicators for the data.
##' @param cens_ind_right the right censoring indicators for the data.
##' @param cens_vec_1 the lower censoring values.
##' @param cens_vec_2 the upper censoring values.
##' @param sim Set to \code{TRUE} if this is a simulation; the file name
##'   containing results for, say `isim=3`, has "3-" appended to the beginning.
##' @param isim Simulation number.
##' @param seedtab A table containing seeds (7 columns)
...
##'
##' @export
one_job_flowcut <- function(ialpha, ibeta, ifold, irep, folds, destin,
                    mean_lambdas, prob_lambdas,
                    ## For simulations
                    sim = FALSE, isim = 1,
                    seedtab = NULL,
     ...
     identical(rownames(X), 1:nrow(X))) # case 2
  {
    ...
    train.dat = ylist[train.inds]
    train.cens_ind_left = cens_ind_left[train.inds]
    train.cens_ind_right = cens_ind_right[train.inds]
    
    train.count = countslist[train.inds]
    train.X = X[train.inds, , drop = FALSE]
  } else { # cases 1 & 3
    train.dat = ylist[-test.inds]
    train.cens_ind_left = cens_ind_left[-test.inds]
    train.cens_ind_right = cens_ind_right[-test.inds]
    train.count = countslist[-test.inds]
    train.X = X[-test.inds,, drop=FALSE]
  }
  ...
  # cases 1, 2, and 3
  test.dat = ylist[test.inds]
  test.cens_ind_left = cens_ind_left[test.inds]
  test.cens_ind_right = cens_ind_right[test.inds]
  test.count = countslist[test.inds]
  test.X = X[test.inds,,drop=FALSE]
  
  #debug(make_cvscore_filename)
  ## Check whether this job has been done already.
  filename = make_cvscore_filename(ialpha, ibeta, ifold, irep, sim, isim)
  if(file.exists(file.path(destin, filename))){
    cat(filename, "already done", fill=TRUE)
  ...
  args$cens_indicator_list_left <- train.cens_ind_left
  args$cens_indicator_list_right <- train.cens_ind_right
  args$cens_lim_vec_lower <- cens_vec_1
  args$cens_lim_vec_upper <- cens_vec_2
  ...
  tryCatch({
    ...
    argn <- lapply(names(args), as.name)
    names(argn) <- names(args)
    call <- as.call(c(list(as.name("flowcut_once")), argn))
    res.train = eval(call, args)
...
##' (Note, \code{nrep} is not an input to this function.)
##'
##' @inheritParams one_job_flowcut
##'
##' @export
one_job_refit_flowcut <- function(ialpha, ibeta, destin,
                          mean_lambdas, prob_lambdas,
                          seedtab = NULL,
                          ## The rest that is needed explicitly for flowmix_once()
                          ylist, countslist, X,
  ...
  for(irep in 1:nrep){
    ...
    if(file.exists(file.path(destin, filename))){
      ...
      args$cens_indicator_list_left   <- cens_ind_left
      args$cens_indicator_list_right  <- cens_ind_right
      args$cens_lim_vec_lower    <- cens_vec_1
      args$cens_lim_vec_upper     <- cens_vec_2
...
##'   from estimation altogether.
##'
##' @param ... Additional arguments to flowcut().
##' @inheritParams flowcut_once
##'
##' @return No return.
##'
##' @export
